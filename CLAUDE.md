...
5. **qlib 读时不复权，复权是 CSV 数据源的责任**——读层裸读 `factor` 列不自动施加；取原始价用 `$close/$factor`（`fork-docs/DATA_SPEC.md §7`）。
